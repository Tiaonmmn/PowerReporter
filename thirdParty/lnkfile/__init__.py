...
class lnk_file(object):
            ...
            try:
                ...
                if self.loc_information['locationFlags'] & 0x0001:
                    ...
                    if self.loc_information['VolumeIDAndLocalBasePath']['rdriveType'] < len(self.DRIVE_TYPES):
                        self.loc_information['VolumeIDAndLocalBasePath']['driveType'] = self.DRIVE_TYPES[
    ...
    def parse_environment_block(self, index, size):
        ...
        self.extraBlocks['ENVIRONMENTAL_VARIABLES_LOCATION_BLOCK']['variable_location'] = self.clean_line(
            self.indata[index + 8: index + 8 + size])
    ...
    def parse_distributedTracker_block(self, index, size):
        ...
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK']['version'] = struct.unpack('<I', self.indata[index + 12: index + 16])[
            0]
        ...
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK']['droid_volume_identifier'] = self.indata[
                                                                                        index + 32: index + 48].encode('hex')
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK']['droid_file_identifier'] = self.indata[index + 48: index + 64].encode(
            'hex')
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK']['birth_droid_volume_identifier'] = self.indata[
                                                                                              index + 64: index + 80].encode(
            'hex')
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK']['birth_droid_file_identifier'] = self.indata[
                                                                                            index + 80: index + 96].encode('hex')
    ...
    def print_lnk_file(self):
        ...
        try:
            print("\tCreation Timestamp: %s" % (
                datetime.datetime.fromtimestamp(self.lnk_header['creation_time'] / 100000000)).strftime(
                '%Y-%m-%d %H:%M:%S'))
            print("\tModified Timestamp: %s" % (
                datetime.datetime.fromtimestamp(self.lnk_header['modified_time'] / 100000000)).strftime(
                '%Y-%m-%d %H:%M:%S'))
            print("\tAccessed Timestamp: %s" % (
                datetime.datetime.fromtimestamp(self.lnk_header['accessed_time'] / 100000000)).strftime(
                '%Y-%m-%d %H:%M:%S'))
