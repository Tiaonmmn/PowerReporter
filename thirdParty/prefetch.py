...
class Prefetch(object):
    ...
    def parseHeader(self, infile):
        ...
        executableName = executableName.split("\x00\x00".encode())[0]
        self.executableName = executableName.replace("\x00".encode(), "".encode())
    ...
    def volumeInformation23(self, infile):
        ...
        while count < self.volumesCount:
            ...
            infile.seek(self.volumesInformationOffset + self.volPathOffset)
            volume = {}
            volume["Volume Name"] = infile.read(self.volPathLength * 2).replace("\x00".encode(), "".encode())
            volume["Creation Date"] = self.convertTimestamp(self.volCreationTime)
            volume["Serial Number"] = self.volSerialNumber
            self.volumesInformationArray.append(volume)
    ...
    def getFilenameStrings(self, infile):
        ...
        for i in self.filenames.split("\x00\x00".encode()):
            self.resources.append(i.replace("\x00".encode(), "".encode()))

    def convertTimestamp(self, timestamp):
        # Timestamp is a Win32 FILETIME value
        # This function returns that value in a human-readable format
        return (datetime(1601, 1, 1) + timedelta(microseconds=timestamp / 10.)).strftime("%Y %m %d - %H:%M:%S")
    ...
    def directoryStrings(self, infile):
        ...
        while count < self.dirStringsCount:
            ...
            directoryString = infile.read(stringLength).replace("\x00".encode(), "".encode())
    ...
    def convertFileReference(self, buf):
        # byteArray = map(lambda x: '%02x' % ord(x), buf.decode())
        byteArray = map(lambda x: '%02x' % ord(chr(x)), buf)
        byteArrayList = list(byteArray)
        byteString = ""
        for i in byteArrayList[::-1]:
            byteString += i
    ...
    def prettyPrint(self):
        ...
        print("Executable Name: {}\n".format(self.executableName.decode()))
        ...
        for i in self.volumesInformationArray:
            print("    Volume Name: " + i["Volume Name"].decode())
    ...
    def decompress(self, infile):
        ...
        with open(infile, 'rb') as fin:
            ...
            if crcck:
                ...
                if crc != file_crc:
                    sys.exit('{0} Wrong file CRC {1:x} - {2:x}!'.format(infile, crc, file_crc))
...
#                             # except:
#                             # print("[ - ] {} could not be parsed".format(i))
#                             print("{},{},{},{},{}".format(p.timestamps[0], p.mftSeqNumber, p.mftRecordNumber,
#                                                           p.executableName, p.runCount))
#                         else:
