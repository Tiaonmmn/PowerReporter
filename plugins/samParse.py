import struct, datetime, subprocess, os
...
class samParse:
    ...
    def binary_to_sid(self, binary_data):
    ...
    def sid_to_username(self, sid, software):
        try:
                    ...
                    for i in range(11, 0, -1):
                        ...
                        if tmp == 257:
                            if struct.unpack("<B", data[offset:offset + 1])[0] == 0:
                                offset = offset + 1
                            results['groups'][groupname]['Members'] += self.binary_to_sid(data[offset:offset + 12])
                            ...
                            results['groups'][groupname]['Members'] += self.binary_to_sid(data[offset:offset + 28])
...
def userAccountParse(mountDir=None, volumeInfo=None):
    parser = samParse()
    ...
    if "FAT" or "NTFS" in output.split(" ")[0]:
        os.chdir("%s/%s" % (mountDir, output.split(" ")[2]))
    ...
    if sam == 1:
        if system == 1:
            samdump2_output = subprocess.getoutput(
