...
def main():
    ...
    if args.disk_mounter not in ('xmount', 'auto') and args.read_write:
        print(col("[!] {0} does not support mounting read-write! Will mount read-only.".format(args.disk_mounter),
                  'yellow'))
    ...
    if args.disk_mounter not in ('auto', 'dummy') and not _util.command_exists(mount_command):
        ...
        sys.exit(1)
            ...
            map(_util.command_exists, ('xmount', 'affuse', 'ewfmount', 'vmware-mount',
                                       'avfsd'))):
        ...
        try:
            ...
            if args.reconstruct:
                ...
                try:
                    ...
                    for disk in p.disks:
                        failed.extend(
                            [x for x in disk.volumes if 'bindmounts' not in x._paths and x.mountpoint and x != root])
                    if failed:
